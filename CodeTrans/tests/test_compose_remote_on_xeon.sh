...
function start_vllm_litellm_server() {
    cd $WORKPATH/tests/vllm-ci-server
    ...
    n=0
    until [[ "$n" -ge 100 ]]; do
        docker logs litellm-proxy > ${LOG_PATH}/litellm_proxy_start.log 2>&1
        if grep -q complete ${LOG_PATH}/litellm_proxy_start.log; then
            break
        fi
        sleep 5s
        n=$((n+1))
...
function start_services() {
    ...
    n=0
    until [[ "$n" -ge 100 ]]; do
        docker logs codetrans-xeon-backend-server > ${LOG_PATH}/backend_service_start.log 2>&1
        if grep -q complete ${LOG_PATH}/backend_service_start.log; then
            break
        fi
        sleep 5s
        n=$((n+1))
...
function stop_vllm_litellm_server() {
    cd $WORKPATH/tests/vllm-ci-server
...
function main() {
    ...
    echo "::group::start_vllm_litellm_server"
    start_vllm_litellm_server
    echo "::endgroup::"
